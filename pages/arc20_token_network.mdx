...
<Callout type="warning">
  **Concept and roadmap, not a live product.** There is no production Token
  Factory, arbitrary-token payment adapter, automatic revenue-share rail, or
  per-generation JobAnchor publisher today. The capabilities below describe a
  proposed network built on existing Grid and contract primitives; do not use
  this page as a deployment or investment claim.
</Callout>

The AIPG Token Factory is a toolkit for **developers who want to launch their own
networks and tokens on top of AIPG**. Instead of building AI infrastructure from
scratch, you deploy a token that taps into the models AIPG already serves — and
build your own product, economy, and community around it.
...
| **Job Anchoring** | Proposed batched commitments for eligible completed jobs |
...
- **GPU Workers**: Would receive the settlement assets defined by the future rail
...
7. **Settlement**: The future rail pays the worker and applies the configured revenue split
...
├─────────────────────────────────────────┤
│ Meanwhile:                              │
│ → Future rail settles worker + treasury │
...
console.log(job.status, job.merkleProof);
```

For generation via the API, users can also call the Grid Generation API directly:
...
A: Not defined. There is no deployed factory or approved fee schedule; Base gas
and any future protocol fee would be shown at transaction time.

**Q: How do workers get paid?**
A: The live Grid currently pays workers through its AIPG payout rail. A Token
Factory would require a separately audited conversion, reservation, settlement,
and revenue-share path before arbitrary tokens could fund worker payouts.
