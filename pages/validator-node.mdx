...
Source and local Docker installs are available now. Downloadable binaries are
not published yet. When `v0.1.0-preview` passes the production gate, the exact
release will carry a provenance-attested installer and checksum manifest:
...
- Production core currently enables targeted text probing only through a
