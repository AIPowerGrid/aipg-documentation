...
> **Design flow only.** This is not the path used by production requests today.
> Claims in the prototype are unsigned and peer identity is not yet durable, so
> this flow must not be used for billed or reward-bearing work.
