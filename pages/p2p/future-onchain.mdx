...
<Callout type="info">
This page describes the **future vision** for a fully decentralized, on-chain
AI Power Grid. The current default-off P2P source prototype is exploratory work
toward that goal, not an operating network or supported beta.
</Callout>
...
The values and interfaces below are illustrative design sketches, not deployed
policy. In particular, `MIN_STAKE` has not been finalized.
...
The following uses 1,000 AIPG only to demonstrate how a per-worker bond scales;
it is not a proposed or active minimum:
...
```
┌─────────────────────────────────────────────────────────────────┐
│  CERTIFIED DETERMINISTIC IMAGE WORKFLOW                         │
│                                                                 │
│  • Pin model, workflow, dependencies, params, seed, and version │
│  • Re-execute on a compatible reference implementation         │
│  • Compare exact bytes only when certification promises them   │
│  • Otherwise use an approved perceptual/feature tolerance      │
│                                                                 │
│  General image/video generation is not automatically           │
│  deterministic or objectively correct.                         │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
