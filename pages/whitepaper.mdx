...
> **Historical archive:** This whitepaper was written during AIPG's original
> Ravencoin-fork era. It is preserved as project history, not as a description
> of the current network, economics, validator requirements, or roadmap. AIPG
> now runs on **Base L2** with a different architecture. See [The Grid](/grid-overview)
> and [Validators](/validators) for current behavior.
> Statements below use the original proposal's future tense and must not be
> quoted as current product behavior.
...
The original proposal paired validators with a Proof-of-Stake (PoS) mechanism
and contemplated an approximately 10,000 AIPG requirement. That mechanism and
amount are **not current network policy**; validator staking and economic
authority are not live, and any future requirement remains to be finalized.
...
The original roadmap proposed validator consensus backed by an approximately
10,000 AIPG stake. The current validator system instead begins with
assignment-bound evidence and no economic authority; staking, rewards,
slashing, and final consensus policy remain future work.
