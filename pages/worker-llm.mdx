...
description: Run an LLM worker on AIPG. No bond required today; completed work contributes to the hourly AIPG payout split.
---
...
Run LLMs on The Grid. Earn AIPG through the current worker payout rail for
powering chat and text generation.

<Callout type="info">
  **No bond required to run a worker today.** You can connect a worker and start
  serving with just an API key. A planned on-chain bonding mechanism (the
  `WorkerRegistry` Diamond facet) is in development and **not yet required**.
  Its production bond amount and slashing policy are not finalized. See
  [Run a Node](/run-a-node) for the bigger picture.
</Callout>
...
5. The job records den work units used for the current hourly AIPG payout split
...
Sign in with Google, GitHub, or a wallet at
[console.aipowergrid.io](https://console.aipowergrid.io) and create a key.

### 3. Download the Worker

Grab the binary for your platform from [Releases](https://github.com/AIPowerGrid/grid-text-worker/releases):
...
4. The job contributes den to your share of the current hourly AIPG payout

### What Earns Most

- **Popular models** — gpt-oss-120b, qwen3-27b get the most traffic
...
| Resource | URL |
|----------|-----|
| **Repository** | [github.com/AIPowerGrid/grid-text-worker](https://github.com/AIPowerGrid/grid-text-worker) |
| **Releases** | [github.com/AIPowerGrid/grid-text-worker/releases](https://github.com/AIPowerGrid/grid-text-worker/releases) |
