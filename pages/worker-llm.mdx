...
description: Run an LLM worker on AIPG. No bond required today — serve chat completions and earn per request.
---
...
Run LLMs on The Grid. Earn USDC + AIPG for powering chat and text generation.
...
<Callout type="info">
  **No bond required to run a worker today.** You can connect a worker and start
  ...
  `WorkerRegistry` Diamond facet, ~1,000 AIPG, slashable for bad results) is in
  development and **not yet required**. See [Run a Node](/run-a-node) for the
  bigger picture.
</Callout>
...
Register at [console.aipowergrid.io](https://console.aipowergrid.io)
