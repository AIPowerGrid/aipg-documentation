...
For real-time token streaming via WebSocket (instead of HTTP polling):
...
Streaming workers connect via WebSocket and deliver tokens in real time to clients using the `/v1/chat/completions` endpoint. [Learn more →](/streaming-api)
